...
from pathlib import Path # Lo vamos a usar para manejo de archivos multiplataforma ya que la BD nos estaba genetrando novedad al conectarse
...
def conectar():
    DB_PATH.parent.mkdir(exist_ok=True) # Esta parate la tuvimos que implementar ya que trataba de conectarse a la BD  pero aun no teniaos la ruta (esto se encarga de crearla si no exixte)
